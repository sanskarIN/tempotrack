...
This guard intentionally checks alignment, not whether a new Gradle version is compatible with every project plugin. Compilation/tests and an explicit Kotlin/AGP compatibility review remain required for a toolchain upgrade.
...
It initializes CodeQL v4 for `java-kotlin`, installs JDK/Android SDK/Gradle 9.5.0, builds Android debug + Desktop Kotlin, then runs analysis.
...
All platform release jobs use Gradle 9.5.0 through `gradle/actions/setup-gradle@v5`; the repository alignment guard protects that pin from drifting away from CI/bootstrap metadata.
