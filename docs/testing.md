...
The guard requires the wrapper distribution, Unix/Windows fallback launchers, and every GitHub workflow using `gradle/actions/setup-gradle` to use one Gradle version. It also requires a pinned distribution SHA-256 plus positive wrapper retry/backoff settings. This catches partial toolchain upgrades before they become platform-specific build failures.

A passing alignment guard does not prove that a newly selected Gradle release is supported by the pinned Kotlin or Android Gradle Plugin versions. Review compatibility before changing the pin, then use compilation/tests as the final evidence.
