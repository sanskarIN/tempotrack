...
The guard reports drift between wrapper metadata, Unix/Windows launchers, CI, CodeQL, and release automation. If all pins match but compilation still fails after a version change, review the Kotlin/Gradle/AGP compatibility matrix; alignment alone does not prove that the selected versions are supported together.
